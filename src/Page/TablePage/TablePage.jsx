...
const TablePage = ({ fill }) => {
  ...
  const getDateUser = () => {
    apiRoot.get(`/v1/get/workers/${TodayDate}`,
      {
        headers: {
          "Authorization": `${token}`
        }
        ...
        <Row>
          <Col md="6">
            <div className="calendar_menu__day">
              <button onClick={() => prev()}><img src={ArrowLeft} alt="left" /></button>
              <div>
                <p>{`${monday.getDate()} - ${monday.addDays(5).getDate()}`} {`${monthName(monday.getMonth())}`} {`${monday.getFullYear()}`}</p>
              </div>
              <button onClick={() => next()}><img src={ArrowRight} alt="right" /></button>
            </div>
          </Col>
          <Col md="3">
            ...
            <div className="tab_menu">
              {arr.map((item, index) => (
                <ul key={index}
                  ...
                  onClick={() => { setNum(index); setMomentDay(item) }}
                >
                  <li className="calendar_day">
                    <p>{item.getDate()} <br /> <span>{monthName(item.getMonth())}</span> </p>
              ...
              <div className="tab_content">
                <table className="table_menu">
                  ...
                  <tbody className='table_body_user'>
                    {
                      <>
                        {alldata?.data?.filter((item) => item?.worker_enter?.length > 0)?.map((item, b) => (
                          <tr key={b}>
                            <td scope='row'>{b + 1}</td>
                            <th scope='row'>
                              <img src={"http://" + item?.worker_img} alt="" />
                            </th>
                            <td>
                              {item?.worker_name}
                            </td>
                            ...
                            <td>
                              ...
                              >
                                <div>
                                  {item?.worker_status?.length > 0 && <Yellow fill={item?.worker_status} />}
                                </div>
                              </OverlayTrigger>
                            </td>
                            <td>
