...
const History = () => {
  ...
  const [monthValue, setMonthValue] = useState(month.filter(item => item?.end === dv)[0]?.id)
  ...
  const HandleValue = (value) => {
    if(value){
      const id = month.filter(item => item?.end === value?.target.value)[0]?.id
      setMonthValue(id)
    }
  }
  const year = moment().format('YYYY [escaped] YYYY')?.slice(0, 4)
  const downloadFile = () => {
    apiRoot.get(`/v1/download/${year}-${monthValue}`, {
      headers: {
        "Authorization": `${token}`
      }
    }).then((res)=>{
  ...
  const onSubmit = () => {
    if(!monthDate){
      apiRoot.get(`/v1/worker/month/${year}-${monthValue}`,
    {
      headers: {
        "Authorization": `${token}`
      }
  ...
  const onSubmit2 = () => {
      apiRoot.get(`/v1/worker/two_date/${firstday}/${lastday}`,{
      headers: {
        "Authorization": `${token}`
      }
    }).then((res)=>{
  ...
  const [defoultMonth, setDefoultMonth] = useState(month.filter(item => item?.end === dv)[0]?.name)
        ...
        <div className="history_header">
          <form action="/">
            <div className="form_group">
              <input type="checkbox" id="check1" name="month" checked={!monthDate} onChange={() => setMonthDate((prev) => !prev)} />
              <label htmlFor="check1">{t("history.h1")}</label>
            </div>
            <div className="form_group">
              <input type="checkbox" id="check2" name="date" checked={monthDate} onChange={() => {
              ...
              {!monthDate &&
                <select onChange={HandleValue} defaultValue={{defoultMonth}}>
                  ...
                  {month?.map((item, index) => (
                    <option key={index} value={item?.end} selected="selected">{item?.name}</option>
                  ))}
                </select>
              }
            </div>
        ...
        <div className="history_table">
          <table className="table_menu">
            <thead className='table_head_user'>
              <tr>
                <th scope="col-md-2" className='time'>{t("worker.w1")}</th>
